...
public class SelectOneMain {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 2. 집계 함수의 결과를 조회
			String sql = "SELECT BOARD_NO, TITLE, CONTENT, HIT, CREATE_DATE FROM BOARD WHERE BOARD_NO = 2";			// 1삭제 했으므로 조회되지 않음.
			...
			if(rs.next()) {		// 스캔 성공히면(조회 결과가 있으면)
				...
				/*
					...
					|     2    | 공지2 |  내용 2 |  0  |   22/09/07  | <= rs.next() 호출 시 rs 포인터의 위치 |
