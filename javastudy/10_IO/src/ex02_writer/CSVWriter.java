...
public class CSVWriter {
	...
	public static void main(String[] args) {
		...
		// 2. 콤마로 분리된 값들
		// 3. 확장자 .csv인 파일(기본 연결프로그램-엑셀, 메모장으로도 오픈 가능)
		...
		try {
			fw = new FileWriter(file, StandardCharsets.UTF_8); // charter set (UTF-8)파일을 사용해야 한글이 깨지지 않음.
			bw = new BufferedWriter(fw);
					for(int i = 0, length = list.size(); i < length; i++) { 	// list에서만 사용하는 방식
