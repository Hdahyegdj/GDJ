...
public class Main2_JSON {
	...
	public static void m2() {
		...
		StringBuilder sb = new StringBuilder();
		try(BufferedReader br = new BufferedReader(new FileReader(file))) {	// 텍스트 파일 읽어드릴때는 Reader를 사용 //BufferedReader는 혼자 사용 못함으로 다른 reader와 함께 써야함.
			String line = null;
	...
	public static void m4() {
		...
		for(int i = 0; i < item.length(); i++) {
			...
			System.out.println(element.get("category") + " : " + element.getString("obsrValue"));
	...
	public static void m6() {
		...
		JSONArray columns = header.getJSONArray("columns");	//[] 는 json array 타입
		...
		for(int i = 0; i < columns.length(); i++) {
			...
			// map.put(columns.getString(0), item.get(p[0]));
			// map.put(columns.getString(1), item.get(p[1]));
		}
	...
	public static String m7() {		// String 반환 타입으로 변경
