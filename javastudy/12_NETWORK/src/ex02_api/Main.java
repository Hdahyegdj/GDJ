...
public class Main {
	...
	public static void m1() {
		...
		try {
			...
			con.setRequestMethod("GET");	// 보안이 필요하지 않은 방식(get)
	...
	public static void m4() {
		...
		File file = new File("C:\\storage", "api2.xml");	// "api2.xml" 파일 전체를 doc이라고 함.
		...
		try {
			...
			StringBuilder sb = new StringBuilder();
			NodeList items = root.getElementsByTagName("item"); 	// 태그 이름으로 찾기	//getElementsByTagName "s"가 들어가면 반환타입이 List 여러개 임.
	...
	public static void m5() {
		...
		// 입력 스트림 생성
		// 1. 서버가 보낸 데이터를 읽어야 하므로 입력 스트림이 필요
		// 2. 서버와 연결된 입력 스트림은 바이트 스트림이므로 문자 스트림을 변환해야 함.
		BufferedReader reader = null;
		StringBuilder sb = new StringBuilder();
		try {
	...
	public static void m6() {
		...
		try {
			...
			for(int i = 0; i < items.getLength(); i++) {
				Element item = (Element)items.item(i); // Node -> Element 타입으로 다운캐스팅 // Node 태그, 줄바꿈 포함 / Element 태그   : 여기서 Node는 부모, Element는 자식 관계이다.
