...
public class ServerMain {
	...
	public static void main(String[] args) {
		...
		try {
			...
			InetSocketAddress address = new InetSocketAddress("localhost", 9090);    // 실제 주소 창  http://localhost:9090
			...
			try {
				if(serverSocket.isClosed() == false) { 		// 서버가 닫히지 않았으면
					serverSocket.close();		// 서버 닫힘
				}
			} catch(IOException e) {
				e.printStackTrace();
			}
