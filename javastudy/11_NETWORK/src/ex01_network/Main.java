...
public class Main {
	...
	public static void m1() {
		...
		// 	  서버경로 search.naver
		//    파라미터(서버 경로) = 값 & 파라미터 = 값		where= nexearch&sm=top
	...
	public static void m2() {		// connection 하는 법 암기 

		// HttpURLConnection
		// 1. 웹 접속을 담당하는 클래스
		// 2. URL 클래스와 함께 사용
		// 3. URLConnection 클래스의 자식 클래스
		// 4. URL 클래스의 openConnection() 메소드를 호출해서 HttpURLConnection 클래스 타입으로 
		...
		try {
			...
			HttpURLConnection con = (HttpURLConnection)url.openConnection();  // 자식 = 부모 관계이므로 다운 캐스팅() 필요
			
			// HTTP 응답 코드
			// 1. 200 : 정상
			// 2. 40X : 요청이 잘못 됨(사용자 잘못)
			// 3. 50X : 서버 오류
	...
	public static void m3() {
		...
		try {
			...
			fw.write(sb.toString());     // String 내보내기
