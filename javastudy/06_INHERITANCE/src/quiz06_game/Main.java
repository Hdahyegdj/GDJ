...
public class Main {
	...
	// 공격 차례 정하기
	boolean myTurn = Math.random() < 0.5; // 절반의 확률로 내가 공격할수도 안할수도 있다.

	
	// 한 대씩 주고 받기
	// 두 유닛이 모두 살아있으면 계속 싸움
	while(unit1.isAlive() && unit2.isAlive()) {	// while(unit1.getEnergy() > 0 && unit2.getEnergy() > 0)  // 둘 다 살아 있으면 
		if(myTurn) {
			...
			unit2.attack(unit1);	// unit2가 unit1을 공격한다. (마리은 40% 확률로 상대를 한 번에 죽임.)
		}
		myTurn = !myTurn;
		}
