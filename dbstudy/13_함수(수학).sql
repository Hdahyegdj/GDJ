...
-- 7. 정수로 내림
--    FLOOR(A) : 실수 A를 정수를 내림
SELECT FLOOR(1.1), FLOOR(-1.1) FROM DUAL;

-- 8. 원하는 자리수로 절사
--    TRUNC(A, [DIGIT]) : 실수 A를 DIGIT 자리수로 절수, DIGIT 생략하면 정수로 절사     ([대괄호] 생략 가능)
SELECT
        TRUNC(1.9999)       -- 1
      , TRUNC(1.9999, 1)    -- 1.9
      , TRUNC(1.9999, 2)    -- 1.99
...
-- 9. 원하는 자리수로 반올림
--    ROUND(A, [DIGIT]) : 실수 A를 DIGIT 자리수로 반올림, DIGIT 생략하면 정수로 반올림
SELECT
        ROUND(145.45)       -- 145
      , ROUND(145.45, 1)    -- 145.5
      , ROUND(145.45, -1)   -- 150(소수점 기준)
FROM
    DUAL;
...
--    4) 정수      : CEIL(값 * 1) / 1                0
--    5) 일의자리  : CELI(값 * 0.1) / 0.1            -1
--    6) 십의자리  : CELI(값 * 0.01) / 0.01          -2
--    7) 백의자리  : CELI(값 * 0.001) / 0.001        -3
--    일반화       : CELI(값 * POWER(10, DIGIT)) / POWER(10, DIGIT)

SELECT
        CEIL(1.111 * POWER(10, 1)) / POWER(10, 1)      -- 1.2
      , CEIL(1.111 * POWER(10, 2)) / POWER(10, 2)      -- 1.12
      , CEIL(11111 * POWER(10, -1)) / POWER(10, -1)     -- 11120
      , CEIL(11111 * POWER(10, -2)) / POWER(10, -2)    -- 111200
    FROM
        DUAL;
