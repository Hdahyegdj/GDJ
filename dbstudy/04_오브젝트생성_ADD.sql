...
-- 외래키 추가
ALTER TABLE EMPLOYEE
    ADD CONSTRAINT FK_EMPLOYEE_DEPARYMENT FOREIGN KEY(DEPT_NO)
...
-- 기본키 제거(자식이 부모를 참조로 하기 때문에 자식부터 삭제해야함(테이블 삭제와 동일))
-- FK에 의해서 참조되고 있는 PK는 제거 할 수 없음.
ALTER TABLE DEPARTMENT 
    DROP CONSTRAINT PK_DEPARTMENT;  -- EMPLOYEE의 FK를 먼저 제거해야 실행 할 수 있음.
    
ALTER TABLE DEPARTMENT
    DROP PRIMARY KEY;   -- 이름을 모르는 경우 사용
