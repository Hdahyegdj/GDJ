...
DESCRIBE DUAL;      -- 테이블 구조 확인(내림차순과 키워드가 동일)
...
-- 1. 숫자로 변환(TO_NUMBER)
SELECT '100', TO_NUMBER('100')  -- 좌측 : 문자, 우측 : 숫자
    FROM DUAL;
...
SELECT 1 + '1'     -- SELECT 1 + TO_NUMBER('1')로 변환하여 처리  -- 숫자 1과 문자 1 더하기(값은 2가 나옴)
  FROM DUAL;
  
-- '문자'와 '문자' 연산도 모두 숫자로 바꿔서 처리
SELECT '1' + '1'    -- SELECT TO NUMBER('1') + TO NUMBER('1')
  FROM DUAL;


/*
EMP_NO는 문자열이라 가정하여 가장 성능이 낮은 조건은?  정답은 1,2(연산자 왼쪽에는 함수가 들어가지 않는게 좋음) (1,2 와 3,4는 동일)
...
SELECT 
      ...
      , TO_CHAR(1234,  '9,999')  -- '1,234'      -- 천단위 구분 기호 표시
      , TO_CHAR(12345, '9,999')  -- '#####'     -- 숫자가 5자리인데 4자리로 표시하라는 명령이므로 표시 값이 #####으로 출력됨
...
SELECT EMP_NO, NAME, DEPART, POSITION, GENDER, HIRE_DATE, SALARY
  FROM EMPLOYEE
 WHERE TO_DATE(HIRE_DATE, 'YY/MM/DD') BETWEEN TO_DATE('90/01/01', 'YY/MM/DD') AND TO_DATE('31/12/99', 'DD/MM/YY');  -- 날짜를 문자타입(VARCHAR2)으로 표현하는 경우가 많음 > 성능 문제
