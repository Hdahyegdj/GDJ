...
-- 5. 테이블 이름 : ALTER TABLE 테이블 RENAME TO NEW이름     (여기서 테이블이 OLD이름이기 때문에 작성 필요가 없다.)
...
ALTER TABLE CUSTOMER
    ADD GRADE VARCHAR2(6 BYTE) CHECK(GRADE IN('VIP', 'GOLD', 'SIRVER'));    --TEXT는 ''를 사용
...
ALTER TABLE BANK
    MODIFY BANK_NAME VARCHAR2(15 BYTE) NULL;    -- 타입 생략은 불가능 함으로 크기는 반드시 기입(동일 크기)해야함.
