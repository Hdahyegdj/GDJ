...
/*
    서브쿼리
    1. SUB QUERY
    2. 메인쿼리(MAIN QUERY)에 포함하는 허위쿼리(SUB QUERY)
...
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
  FROM EMPLOYEE
 WHERE POSITION = (SELECT POSITION          -- 단일 행 서브쿼리이므로 연산자 =를 사용, 하위쿼리는 메인퀴리와 동등비교(=)되므로 반드시 POSITION을 반환시켜줘야함.
                     FROM EMPLOYEE
...
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
  FROM EMPLOYEE
 WHERE (SYSDATE - HIRE_DATE) >= (SELECT AVG(SYSDATE - HIRE_DATE)     -- 근속시간 >= 평균근속기간;
                                   FROM EMPLOYEE);
...
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
  FROM EMPLOYEE
 WHERE POSITION IN (SELECT POSITION         -- 다중 행 서브쿼리에 동등 비교는 IN 연산으로 수행해야 함
                     FROM EMPLOYEE 
                    WHERE DEPART = 2);      -- DEPART가 PK/UNIQUE가 아니기 때문에 다중 행 서브쿼리


-- TIP : 단일 행/다중 행 상관 없이 동등 비교는 IN 연산으로 수정 가능
-- POSITION = '과장'과 POSITION IN ('과장')은 동일


-- 7. 부서명이 '영업부'인 부서에 근무하는 사원 조회하기
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
  FROM EMPLOYEE
 WHERE DEPART IN (SELECT DEPT_NO       -- 영업부의 부서번호를 찾아옴
                      FROM DEPARTMENT
                     WHERE DEPT_NAME = '영업부');      -- DEPT_NAME이 PK/UNIQUE가 아니기 때문에 다중 행 서브쿼리
...
SELECT DEPT_NO, DEPT_NAME, LOCATION
  FROM DEPARTMENT
 WHERE DEPT_NO IN(SELECT DEPART   --과장들의 부서번호가 여러개이기 때문에 다중 행 서브쿼리가 됨.
                    FROM EMPLOYEE
...
-- 어떤 급여(2000000, 50000000)이든 하나의 급여보다 많이 받으면 조회하기
...
-- SALARY가 20000000보다 크거나, 50000000보다 크면 됨(OR 개념)
 
 
-- 10. 부서번호가 1인 부서에 근무하는 사원들의 급여보다 더 많은 급여를 받는 사원 조회하기
-- 모든 급여(2000000, 50000000)와 비교해서 많이 받으면 조회하기
...
-- SALARY가 20000000보다 크고, 50000000보다 크면 됨(AND 개념)                     
-- 따라서 최대급여 50000000보다 크면 만족하는 상황임
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
  FROM EMPLOYEE
 WHERE SALARY > (SELECT MAX(SALARY)
                      FROM EMPLOYEE
...
SELECT E.EMP_NO, E.NAME, E.DEPART, D.DEPT_NAME
  FROM DEPARTMENT D INNER JOIN EMPLOYEE E
    ON D.DEPT_NO = E.DEPART
 WHERE D.LOCATION =  (SELECT LOCATION      -- (부서번호가 1인 부서의 지역이름)
                        FROM DEPARTMENT
                       WHERE DEPT_NO = 1);
                       
-- 스칼라 서브퀴리 접근

-- 스칼라 서브퀘리는 일치하지 않는 정보를 NULL로 처리함
-- 따라서 스칼라 서브쿼리는 동일한 방식의 조인은 '외부조인'임
SELECT 
        E.EMP_NO
      , E.NAME
      , E.DEPART
...
/* 
    ...
    3. 인라인뷰는 주로 테이블 형식임
    4. 인라인뷰에 별명을 주고 사용함
    5. 인라인뷰에서 조회한 칼럼만 메인 쿼리에서 조회할 수 있음.
    6. SELECT문의 실행순서를 바꿀 때 사용
*/

SELECT A.EMP_NO, A.NAME, A.POSITION   -- 인라인뷰가 조회한 칼럼만 작성할 수 
...
SELECT EMP_NO, NAME
  FROM EMPLOYEE
 WHERE ROWID = 'AAAFBUAABAAALDBAAC';
...
SELECT EMP_NO, NAME
  FROM EMPLOYEE
 WHERE ROWNUM BETWEEN 1 AND 2;  -- WHERE ROWNUM = 1;1에 해당하는 값 가져오기(1이 포함되지 않으면 사용 불가) -- WHERE ROWNUM BETWEEN 1 AND 3; 1과 3사이 값 
 
-- ROWNUM을 1이외의 범위를 조건으로 사용하는 방법
...
SELECT B.ROW_NUM, B.EMP_NO, B.NAME, B.SALARY      -- 별명을 이용해 연봉 높은 순으로 정렬하여 SELECT문은 한번 더 사용해 2번째 높은 연봉 받는 사원을 조회해야함(ROWNUM은 1외의 사용 불가)
  FROM (SELECT ROWNUM AS ROW_NUM, A.EMP_NO, A.NAME, A.SALARY
          FROM (SELECT EMP_NO, NAME, SALARY
                  FROM EMPLOYEE
              ORDER BY SALARY DESC) A) B
 WHERE ROW_NUM = 2;   -- BETWEEN 2 AND 3 : 2번째에서 3번째사이
...
SELECT A.EMP_NO, A.NAME, A.SALARY, A.HIRE_DATE
  FROM(SELECT ROW_NUMBER() OVER(ORDER BY HIRE_DATE) AS ROW_NUM, EMP_NO, NAME, SALARY, HIRE_DATE
         FROM EMPLOYEE) A
...
SELECT CONSTRAINT_NAME FROM USER_CONSTRAINTS WHERE TABLE_NAME = 'EMPLOYEE2'; -- EMPLOYEE2의 제약 조건만 표시
...
CREATE TABLE DEPARTMENT2
    AS (SELECT DEPT_NO, DEPT_NAME, LOCATION
          FROM DEPARTMENT
         WHERE 1 = 2); -- 절대 만족하지 않는 조건(구조만 복사됨)           -- WHERE DEPT_NO = 1); 1번 부서만 복사(0일 경우, 데이터가 없기 때문에 아무것도 복사되지 않음)
...
-- 3. 형식
--    INSERT INFO 테이블_이름(칼럼1, 칼럼2, ...) (서브쿼리)

-- 3. DEPARTMENT 테이블의 모든 행(ROW)DMF DEPARTMENT2 테이블에 삽입
...
UPDATE EMPLOYEE2
   SET NAME = '민경태'
     , GENDER = 'M'         -- UPDATE 2개
 WHERE EMP_NO = '1001';     -- 사원번호 시작 1001

UPDATE EMPLOYEE2        --사원 1004번 정보를 1001로 넘겨주기
   SET NAME = (SELECT NAME FROM EMPLOYEE WHERE EMP_NO = 1004)
     , GENDER = (SELECT GENDER FROM EMPLOYEE WHERE EMP_NO = 1004)
 WHERE EMP_NO = '1001';     
 
 UPDATE EMPLOYEE2        --사원 1003번 정보를 1001로 넘겨주기
...
DELETE
  FROM EMPLOYEES
 WHERE DEPART IN (SELECT DEPT_NO        -- 부서번호 IN (영업부의 부서번호) =은 사용할 수 없음(둘 이상 있을 가능성이 있기 때문에 IN을 사용해야함)  -- 부서이름이 영업부인 사원제거
                    FROM DEPARTMENT2
                   WHERE DEPT_NAME = '영업부');
COMMIT;
