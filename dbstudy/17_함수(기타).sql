...
SELECT 
       DISTINCT DEPARTMENT_ID       -- 동일 부서ID 비표시
     , SUM(SALARY) OVER(PARTITION BY DEPARTMENT_ID) AS 부서별연봉합계
     , FLOOR(AVG(SALARY) OVER(PARTITION BY DEPARTMENT_ID)) AS 부서별연봉평균    -- FLOOR : 소수점 삭제
  ...
  FROM 
       EMPLOYEES
 WHERE
       DEPARTMENT_ID IS NOT NULL;   -- NULL값 삭제(NULL값이 빠지만 INDEX 실행 가능)
    
--     RANK() 함수와 PARTITION BY를 함께 사용하면 그룹 내 순위 구하기 가능함
SELECT
        RANK() OVER(PARTITION BY DEPARTMENT_ID ORDER BY SALARY DESC) AS 부서내연봉순위    -- SALARY 높은 순으로 정렬
      , EMPLOYEE_ID
      , FIRST_NAME
      , LAST_NAME
      , SALARY
...
SELECT
        EMPLOYEE_ID
      , PHONE_NUMBER  
      , DECODE(SUBSTR(PHONE_NUMBER, 1, 3)       -- 첫번째 글자부터 3글자만 가져옴
        , 011, 'MOBILE'
        , 515, 'EAST'
        , 590, 'WEST'
        , 603, 'SOUTH'
...
-- 4. 분기 표현식
-- CASE
--      WHEN 조건식1 THEN 결과값1     --IF와 비슷한 재질
--      WHEN 조건식2 THEN 결과값2
--      ...
--      ELSE 결과값N
-- END
