...
SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME
  FROM EMPLOYEES
 WHERE FRIST_NAME = INITCAP('JAMES');       -- 'JAMES'만으로는 조회 불가(EMPLOYEE 데이터가 첫글자만 대문자 표시임)
...
SELECT
        FRIST_NAME || ' ' || LAST_NAME AS FULL_NAME
...
SELECT
  ...
  FROM
      EMPLOYEES;    --0으로 표시되는 값은 A를 가지고 있지 않음
...
SELECT
      RPAD(SUBSTR(EMAIL, 1, 2), 5, '*')   -- 오른쪽에 *를 채워서 자리로 조회
  FROM
      EMPLOYEES;
