...
/* 주석 */
 ...
 /*
    ...
    NUMBER(p,s) : 정밀도(p), 스케일(s)로 표현되는 숫자 타입
      ...
      NUMBER(2, 2) : 1미만의 소수점 2자리 실수 (ex) 0.12 : 0은 카운트하지 않음)
 */
...
DESCRIBE SYS.USER_CONSTRAINTS;  -- sys. 생략가능(owner는 생략이 가능)

-- USER CONSTRAINTS 테이블의 CONSTRAINTS_NAME 칼럼 확인
SELECT CONSTRAINT_NAME FROM USER_CONSTRAINTS;

-- 1. 제약조건 없이 테이블 만들기
DROP TABLE USER_TBL;  -- 두번 실행하면 에러 발생함으로 삭제하여 재생성하도록함
CREATE TABLE USER_TBL(
    USER_ID VARCHAR2(30 BYTE) NOT NULL PRIMARY KEY,
    USER_PW VARCHAR2(30 BYTE) NOT NULL, -- 필수(NOT NULL)
    USER_NAME VARCHAR2(30 BYTE) NULL,
    USER_AGE NUMBER(3) NULL CHECK(USER_AGE BETWEEN 0 AND 100), --나이가 0~100까지
...
CREATE TABLE USER_TBL(
    ...
    USER_PW VARCHAR2(30 BYTE) NOT NULL, -- 필수(NOT NULL)
    USER_NAME VARCHAR2(30 BYTE) NULL,
    USER_AGE NUMBER(3) NULL, --나이가 0~100까지
    USER_ADDR VARCHAR2(30 BYTE) NULL,
...
CREATE TABLE USER_TBL(
    ...
    USER_PW VARCHAR2(30 BYTE) NOT NULL,  -- 이름없이 생성(SYS_C007074)
    USER_NAME VARCHAR2(30 BYTE) NULL,
