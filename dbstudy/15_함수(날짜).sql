...
-- 1. 현재 날짜와 시간
SELECT SYSDATE, STSTIMESTAMP FROM DUAL;
...
SELECT 
    ...
    , EXTRACT(HOUR   FROM SYSTIMESTAMP) AS 시    -- UTC기준(세계 표준시)
    ...
    , FLOOR(EXTRACT(SECOND FROM SYSTIMESTAMP)) AS 초     -- 내림 사용하여 초 소수점 표기 삭제
  FROM
      DUAL;
