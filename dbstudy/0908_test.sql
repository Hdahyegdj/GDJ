...
SELECT
        RANK() OVER(ORDER BY HEIGHT DESC) AS 가장높은키순위
      , STUDNO    
      , SNAME    
      , USERID    
      , GRADE   
...
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE(SQLCODE);   --오류코드 출력
        DBMS_OUTPUT.PUT_LINE(SQLERRM);   --오류메시지 출력
END MY_PROC;
