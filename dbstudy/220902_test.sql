...
*/
CREATE TABLE NEW_PRODUCT AS SELECT * FROM PRODUCT WHERE 1=2;
COMMIT;
...
SELECT E.EMP_ID, E.EMP_NAME, E.SALARY, D.DEPT_ID, D.DEPT_NAME
 ...
 WHERE E.SALARY = (SELECT MAX(E.SALARY)
                   FROM EMPLOYEE E LEFT OUTER JOIN DEPARTMENT D
                     ON E.EMPLOYEE = D.DEPT_NAME);

-- 문제2
SELECT EMP_ID, EMP_NAME, SALARY
  FROM EMPLOYEE
 WHERE DEPARTMENT IN(SELECT DEPT_ID
                       FROM DEPARTMENT
                      WHERE DEPT_NAME = '총무부');
...
CREATE TABLE USER_BOARD (
    USER_BOARD_NO    NUMBER              NOT NULL CONSTRAINT PK_BOARD PRIMARY KEY,
    USER_TITLE       VARCHAR2(1000 BYTE) NOT NULL,
    USER_CONTENT     VARCHAR2(4000 BYTE) NULL,
    USER_HIT         NUMBER              NULL,
