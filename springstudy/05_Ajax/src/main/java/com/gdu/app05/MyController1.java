...
public class MyController1 {
	...
	// field
	@Autowired	 // Container(root-context.xml)에서 타입(class)이 일치하는 bean을 가져오세요(타입만 같으면 이름이 달라도 상관없음).  // xml: root-context 상에서 bean 작성, java: config 패키지 사용하여 bean 작성
	...
	/*
		@ResponseBody
		안녕, 난 ajax 처리하는 메소드야.
		내가 반환하는 값은 뷰(JSP) 이름이 아니고, 어떤 데이터야(text, json, xml 등)야.
	...
	*/
	...
	public String detail1(HttpServletRequest request, HttpServletResponse response) {
		...
		return str;		// str.jsp 이름으로 해석함으로 @ResponseBody를 이용해서 응답값으로 인식되도록 처리
	}
	...
	return member;	// jackson(로 변환하는 코드르 작성하지 않아도 됨)이 member 객체를 {"id": 아이디, "pw":패스워드} 형식의 JSON으로 바꿔서 전달합니다.
					/*
						추억의 코드
	...
	public Map<String, Object> detail3(Member member) {
		...
		return map;
		// return memberService.execute3(member); 와 동일 코드
	}
