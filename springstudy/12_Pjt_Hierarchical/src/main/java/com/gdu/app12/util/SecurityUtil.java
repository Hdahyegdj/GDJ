...
public class SecurityUtil {
	...
	public String preventXSS(String str) {
		str = str.replace("<", "&lt;");		// "<" 이런 코드가 들어오면 "&lt;"(작다)로 변경
		str = str.replace(">", "&gt;");		// ">" 이런 코드가 들어오면 "&gt;"(크다)로 변경
		str = str.replace("\"", "&quot;");  // "\""(큰따옴표)
		str = str.replace("\'", "&#x27;");
		return str;							// ex) <script>로 작성 시 -> It;Scriptgt;로 나옴(웹상에서)
	}
	...
	public String getAuthCode(int length) {
		...
		for(int cnt = 0; cnt < length; cnt++) {		// length만큼 반복
			if(secureRandom.nextDouble() < 0.5) {	// 50% 확률로
				sb.append( (char)(secureRandom.nextInt(10) + '0') );  	// 48('0')부터 10개의 숫자 중 하나
			} else {
