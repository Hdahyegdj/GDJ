...
public class RequestLoggingAspect {
	...
	// 로거
	private static final Logger LOG = LoggerFactory.getLogger(RequestLoggingAspect.class);		// static final이므로 LOG대문자로 기입

	// 포인트컷 설정
	@Pointcut("within(com.gdu.app08.controller..*)")	// 컨트롤러의 모든 메소드를 포인트컷으로 지정하겠다.(controller 패키지 아래 모든 패키지(..)의 모든 클래스(*))
													    // 컨트롤러의 모든 메소드에서 어드바이스(콘솔에 로그 찍기)가 동작한다.
	public void setPointcut() {} 	// 오직 포인트컷 대상ㅇㄹ 결정하기 위한 메소드(이름: 아무거나, 본문 : 없음)
	
	// 어드바이스 설정
	// 어드바이스 실행 시점
	// @Before, @After, @AfterReturning(정상), @AfterThrowing(에러), @Around(메소드 전후)
	@Around("com.gdu.app08.aop.RequestLoggingAspect.setPointCut()")		// setPointCut() 메소드에 설정된 포인트컷에서 동작하는 어드바이스
	public Object executeLogging(ProceedingJoinPoint joinPoint) throws Throwable {	// @Around는 반드시 ProceedingJoin
