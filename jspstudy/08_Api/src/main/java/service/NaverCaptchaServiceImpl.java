...
public class NaverCaptchaServiceImpl implements NaverCaptchaService {
	...
	// field
	private final String CLIENT_ID = "HaV9ctSujf0MKCRVAUx2";
	private final String CLIENT_SECRET = "1In91HuPGA";

	@Override
	public String getCaptchaKey() {

		// code=0 : "키 발급", code=1 : "사용자 입력값 검증"
		String apiURL = "https://openapi.naver.com/v1/captcha/nkey?code=0";		// apiURL 복사 후, 조건에 맞는 code를 ? 뒤에 기입
		
		// 반환할 key값(네이버API가 제공하는 캡챠키)
		String key = null;
		...
		try {
			...
			while((line = reader.readLine()) != null) {
				sb.append(line);			// sb에는 postman에서 출력된 값이 들어있음
			}
		...
		try {
			...
			if(con.getResponseCode() == 200) {	  // 200 : HttpURLConnection.HTTP_OK
				...
				while((readByte = in.read(b)) != -1) {
					out.write(b, 0, readByte);    // 읽어드린 만큼 반환하는 메소드
				}
