...
public class MemberController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "/member/loginPage.do":
			...
			break;
		case "/member/refreshCaptcha.do":		// ajax 이동이므로 actionforward 사용하지 않음(페이지 변경 x, dir/file name만 변경)
			service.refreshCaptcha(request, response);
			break;		
		case "/member/validateCaptcha.do":
			boolean result = service.validateUserInput(request);
			if(result) {
				af = new ActionForward("/member/success.jsp", false);	 // result가 true 이면
			} else {
