...
<script>
	...
	$(document).ready(function(){
		...
		$('#frm_detail').submit(function(event){
			...
			var eng = $('#eng');
			var math = $('#math');
			if(kor.val() == '' || isNaN(kor.val()) || kor.val() < 0 || kor.val() > 100){		/* isNaN : 숫자가 아니면 true 반환 */  /* kor.val() == '' ||   : 빈문자 처리 */
				alert('국어 점수를 확인하세요.');
				kor.focus();
...
<body>
	<!-- 학번은 SEQ처리 했기 때문에 readonly를 사용하여 보이지만 수정할 수 없도록 처리(평균, 학점은 파생값임으로 수정불가) -->
	<h1>학생 상세 조회</h1>
	<div>
