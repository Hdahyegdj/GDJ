    ...
    public void contextInitialized(ServletContextEvent arg0)  { 
    	...
    	/*
    	  	...
    	  	2. 상세
    	  		...
    	  		3) 시 : 0 ~ 23
    	  		4) 일 : 1 ~ 31
    	  		5) 월 : 0 ~ 11 또는 JAN, FEB, MAR, APR, MAY ...
    	  	...
    	  	4. 작성 예시
    	  		1) 매일 10초마다   : 0/10 * * * * ? (초 분 시 일 월 요일 순서)
    	  		2) 매일 1분마다    : 0 0/1 * * * ?
    	  		3) 금요일 12시마다 : 0 0 12 ? * FRI (일을 지정할 수 없음으로 '?')     // 금요일 토요일 마다 : : 0 0 12 ? * FRI,SAT
    	...
    	try {
    		...
    		Trigger trigger = TriggerBuilder.newTrigger()
    				...
    				.startNow()
    				.withSchedule(CronScheduleBuilder.cronSchedule("0/30 * * * * ?"))		// 30초마다(요일 단위 '?'로 고정)
    				.build();
