		...
		$.ajax({
			...
			success: function(resData){    // resData : {"count": 3, "members": [{}, {}, {}]}
				...
				$.each(resData.members, function(i, member){
					...
					tr += '<td><input type="button" value="조회" class="btn_detail"><input type="hidden" value="' + member.memberNo + '"></td>';			// 동적 요소(js event 16장 참조) // 상세보기는 "btn_detail" 버튼을 눌렀을 때 // id는 동일하게 생성되면 안되므로 class(동일한 class가 있어도 상관x)로 변경하여 사용
					tr += '</tr>';
		...
		*/
		$('body').on('click', '.btn_detail', function(){
			$.ajax({
				...
				data: 'memberNo=' + $(this).next().val(),		// this.next()는 실행되지 않음.(메소드가 다름)		// "hidden"을 앞에 작성하면 next()는 prev()가 되어야함.
				/* 응답 */
				dataType: 'json',
	...
	function fn_registration(){
		...
		$('#btn_add').click(function(){
			...
			$.ajax({
				...
				data: $('#frm_memeber').serialize(),	// serialize() : 폼의 모든 입력 요소를 파라미터로 변환하는 메소드(사용하지 않으면 id하나하나 전부 작성해야함)
