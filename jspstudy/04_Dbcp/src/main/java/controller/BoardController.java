...
import service.BoaradEditService;
...
public class BoardController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "board/add.do":
			...
			break;
		case "board/edit.do":
			service = new BoaradEditService();
			break;
		case "board/modify.do":
			service = new BoardModifyService();
			break;
			
		// 비즈니스 로직이 없는 경우(단순이동(ActionForward 사용해서 어디로 갈지만 명시해주면 됨))
		case "board/write.do":
		...
		// Service 실행
		try {								// BoardListService 상  Exception 처리
			if(service != null) {
