...
public class UploadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(dir.exists() == false) {
			dir.mkdirs();	  // 경로 들어가 있는 디렉토리는 반드시 -s를 사용
		}
		
		// 4. 업로드
		// 		COS 라이브러리가 지원하는 MultipartRequest 객체를 생성하면 업로드가 진행됨
		MultipartRequest multipartRequest = new MultipartRequest( 
				/* HttpSernletRequest */ request,
				/* 업로드 할 경로 */	 realPath,
				/* 최대 크기 */		     1024 * 1024 * 10,
				/* 인코딩 */		     "UTF-8",
		...
		String uploader = multipartRequest.getParameter("uploader");	 // "upload"는 client.html의 name 속성 값임
		...
		long size = file.length(); 	 // 파일 크키(바이트)
		String strSize = new DecimalFormat("#,##0").format(size / 1024);		// "#,##0" : 1000단위 표현 기호
		...
		// 6. 응답 (업로드가 완료되면 해당 주소에 보관됨.  (C:\GDJ\jspstudy\.metadata\.plugins\org.eclipse.wst.server.core\tmp0\wtpwebapps\01_Servlet\load)
