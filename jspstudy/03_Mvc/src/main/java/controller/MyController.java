...
// @WebServlet("/today.do", "/now.do", "/adder.do") 이렇게 하면 둘다 처리가 되는 것은 맞으나 좋은 방법은 아님
@WebServlet("*.do")	 // .do로 끝나는 모든 요청(추천 방법)
...
public class MyController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String command = requestURI.substring(contextPath.length() + 1);	// command : today.do 또는 now.do를 저장한 상태(/03_Mvc는 7글자 임으로 index8번 글자부터 끝까지)
		...
		if(actionForward != null) {		// IOException 처리 후
