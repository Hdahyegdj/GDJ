...
/*
  	...
  	2. 이름이 없는 익명 함수를 사용함
  	3. 실행(런타입)할 때 익명 구현 객체가 만들어지면서 동작함
  	4. 형식
  		(매개변수) -> { 메소드본문 }
  	5. 예시
  		1) (int a) -> { System.out.println(a); }
  		2) (a) -> { System.out.println(a); }	 매개변수의 타입은 작성하지 않는다.
 		3) a -> System.out.println(a); 			 매개변수가 하나이거나, 메소드본문이 하나이면 중괄호를 작성하지 않는다.  // 2)와 3)은 동일 코드
 		4) () -> System.out.println("Hello'); 	 매개변수가 없으면 빈괄호를 작성한다.
 		5) (a, b) -> {
 			System.out.println(a + b);
 			result a + b;						 반환타입의 명시를 하지 않는다.
 		}
