...
/* 
  	JUnit 단위 테스트 (순서 insert(삽입) @before, update(수정) @test, delete(삭제) @after)
...
public class BoardTest {
	...
	public void 목록테스트() {	// 메소드명으로 한글을 많이 사용한다.
		...
		// 목록의 개수가 5개이면 테스트 성공, 아니면 실패 (목록의 개수가 5개라 가정한다.)
		List<Board> boards = BoardDao.getInstance().selectAllBoards();
		assertEquals(3, boards.size());  	// 3개를 기대했는데 몇 개이다.
		
	}
	
	@Test
	public void 상세조회테스트() {
		// 3번 게시글의 제목이 '공지입니다'이면 테스트 성공, 아니면 실패
		/*
		Board board = BoardDao.getInstance().selectBoardByNo(3);
		assertEquals("공지입니다.", board.getTitle());
		*/
	...
	public void 게시글삽입테스트() {
		...
		assertEquals(1, result);  // 삽입 테스트의 경우 성공이면 1이 반환 됨
	}
	...
	public void 게시글수정테스트() {
		...
		board.setBoardNo(4);  // 게시글삽입테스트 결과로 삽입된 게시글번호
	...
	public void 게시글삭제테스트() {
		// 게시글삽입테스트로 테스트로 삽입한 게시글 삭제
		int result = BoardDao.getInstance().deleteBoard(4);
		assertEquals(1, result);
	}
