...
<body>
	...
	<%
		...
		if(cart == null){
			...
			for(int i = 0; i < cart.size(); i++){		// 배열이 아니므로 size 사용(배열일 경우 length)
				String item = cart.get(i).get("item") + "-----" + cart.get(i).get("amount");	// array list에서는 cart.get(i)이 배열의 cart[i]와 동일
