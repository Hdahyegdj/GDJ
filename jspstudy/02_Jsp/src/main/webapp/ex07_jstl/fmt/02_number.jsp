...
<body>
	...
	<%--
		...
		1. DacimalFormat 클래스를 대체하는 태그
		2. value 속성과 pattern 속성과 type 속성 이용
		3. pattern 속성에서 사용하는 패턴은 DacimalFormat과 동일한 패턴임
		4. 자릿수는 반올림 처리함
	 --%>
	 
	 <%-- 천 단위 구분 기호(,) 사용 --%>
	 <h1><fmt:formatNumber value="${n}" pattern="#,##0" /></h1>    <%-- 12,345 --%>
	 <h1><fmt:formatNumber value="${n}" pattern="#,##0.0" /></h1>	
	 <h1><fmt:formatNumber value="${n}" pattern="#,##0.00" /></h1>	<%-- 12,345.68 --%>
	 
	 <%-- 천 단위 구분 기호(,) 없음 --%>
	 <h1><fmt:formatNumber value="${n}" pattern="" /></h1>    
	 <h1><fmt:formatNumber value="${n}" pattern="0.0" /></h1>	
