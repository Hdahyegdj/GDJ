...
<body>
	...
	<%-- 
		1. Jsp의 Binding 영역 4개
			...
			3) request     : httpServeltRequest와 같은 영역
	...
	<%-- 표현언어(EL)를 이용해 속성 접근(일반 자바변수는 EL로 표현할 수 없음(EL로 사용하려면 영역에 저장하여야 사용 가능) --%>
	<div>a : ${a}</div>
	<div>b : ${b}</div>
	<div>c : ${c}</div>
	<div>d : ${d}</div>
	...
	<% 
		...
		pageContext.setAttribute("movie", "미나리");	// 우선 순위 높음(그대로 실행하면 전부 미나리로 나오므로 ${applicationScope.movie}를 사용하여 실행(pageContext는 붙이지 않아도됨.))
	%>
